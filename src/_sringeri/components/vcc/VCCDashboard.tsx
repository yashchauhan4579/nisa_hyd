...
import { apiClient, type VCCStats, type VCCRealtime, type VCCDeviceStats } from '@sringeri/lib/api';
...
import { DateTimeRangePicker, type DateTimeRange } from '@sringeri/components/vcc/DateTimeRangePicker';
import { MultiCameraSelector, type CameraOption } from '@sringeri/components/vcc/MultiCameraSelector';
import { VCCInsights } from '@sringeri/components/vcc/VCCInsights';
import { VCCHeatmap } from '@sringeri/components/vcc/VCCHeatmap';
import { VCCDevicesView } from '@sringeri/components/vcc/VCCDevicesView';
import { VCCReportModal } from '@sringeri/components/vcc/VCCReportModal';
import { utcHourToIST, toIST, getTodayStartIST } from '@sringeri/lib/dateUtils';
...
function VCCDashboardInner() {
  ...
  // Demo: the analytics only ran for ~2 days a while back, so any recent window is
  // empty. Anchor the start far in the past so ALL data shows (the backend GROUP BY
  // only returns buckets that actually have data, so charts stay tight to the data).
  const [dateRange, setDateRange] = useState<DateTimeRange>(() => {
    return { startDate: new Date('2020-01-01T00:00:00Z'), endDate: new Date() };
  });
  ...
  const fetchSingleCamera24h = async (cameraId: string) => {
    ...
    try {
      const end = new Date();
      const start = new Date('2020-01-01T00:00:00Z'); // demo: all data (aggregated by time-of-day)
  ...
  const fetchHeatmapStats = async (silent = false) => {
    try {
      ...
      const end = new Date();
      const start = new Date('2020-01-01T00:00:00Z'); // demo: all data (was last 7 days)
  ...
  const fetchTodayStats = async (silent = false) => {
    try {
      ...
      // Demo: show all data (was today 00:00 IST -> now)
  ...
  // Dedicated rolling-24h HOURLY fetch for the "Detections Over Time" chart
  // (was a 7-day per-day series; per-hour reads better for live traffic).
              ...
              <div className={selectedCameraIds.length > 0 ? "space-y-4" : "col-span-2 space-y-4"}>
                ...
                <Card className="bg-zinc-900/30 border border-white/5 rounded-lg p-4 relative overflow-hidden hover:bg-zinc-900/50 transition-all duration-300">
                  <h2 className="text-lg font-semibold tracking-tight mb-4 flex items-center gap-2 relative z-10">
                    ...
                    {selectedCameraIds.length !== 1 && !chartLoading && (
                      <span className="text-xs text-muted-foreground font-normal ml-1">Last 24h · hourly (IST)</span>
                    )}
                  </h2>
                        ...
                        return (
                          <>
                            ...
                            <div className="absolute inset-0 w-full h-full z-10 pointer-events-none">
                              {points.map((p, index) => {
                                // Hour buckets arrive as UTC timestamps — show them as IST.
                                const rawTs = p.item.hour || p.item.day || p.item.week || p.item.month || '';
                                const rawLabel = p.item.hour
                                  ? (() => {
                                      const d = new Date(String(rawTs).trim().replace(' ', 'T') + 'Z');
                                      return isNaN(d.getTime())
                                        ? String(rawTs)
                                        : d.toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Asia/Kolkata' }) + ' IST';
                                    })()
                                  : String(rawTs);
                                return (
                                  <div
