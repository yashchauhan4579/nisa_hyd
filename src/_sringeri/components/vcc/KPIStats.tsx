...
import type { VCCStats, VCCRealtime } from '@sringeri/lib/api';
...
export function KPIStats({ stats, realtime, totalCameras = 0, isSingleCamera = false }: KPIStatsProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Detection Rate — vehicles/hour because per-minute rounds
                to 0 on this site (typical traffic ≈ 14 vehicles/hour =
                0.2 / min). Falls back to averagePerHour from /vcc/stats
                when realtime.perMinute is empty (no traffic in last 5
                min, which is most of the time). */}
            <Card className="bg-zinc-900/30 border border-white/5 rounded-lg p-3 flex flex-row items-center justify-between relative overflow-hidden group hover:bg-zinc-900/50 transition-all duration-200">
                <div className="flex flex-col gap-0.5 z-10 w-full pr-4">
                    ...
                    <h3 className="text-lg font-bold tabular-nums tracking-tight truncate pr-2 mt-0.5">
                        {(() => {
                            const perHour = realtime?.perMinute
                                ? realtime.perMinute * 60
                                : (stats?.averagePerHour ?? 0);
                            return Math.round(perHour).toLocaleString();
                        })()}
                    </h3>
