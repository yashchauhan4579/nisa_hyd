...
import type { Device, DeviceHeartbeatPoint, MagicBoxRuntimeInfo } from '@irisdrone/lib/api';
import { formatTimeAgo } from '../widgets/utils';
import { computeUptimePercent, UptimeChart } from './UptimeChart';
import { MagicBoxTelemetryPanel } from './MagicBoxTelemetryPanel';
...
function heartbeatStatusBadge(s: string) {
  ...
  return <Badge className="bg-zinc-500/20 text-zinc-400 border border-zinc-500/30 text-xs">{s || '—'}</Badge>;
}
    ...
    <Card className="h-full bg-zinc-900/30 border border-white/5 backdrop-blur-sm flex flex-col overflow-hidden">
      <div className="p-6 border-b border-white/10 shrink-0">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="text-2xl font-bold text-zinc-100">{device.name || device.id}</div>
          <Button
            variant="outline"
            size="sm"
        ...
        <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-zinc-300">
          <span>ID: <span className="font-mono">{device.id}</span></span>
          {device.lastSeen && <span>Last online: {formatTimeAgo(device.lastSeen)}</span>}
          {(() => {
            const wgIp = (device.runtimeInfo?.wg_interface_ip as string | undefined)
              ?? ((device.metadata as Record<string, unknown> | undefined)?.wg_interface_ip as string | undefined);
            return wgIp ? <span>WG IP: <span className="font-mono">{wgIp}</span></span> : null;
          })()}
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <MagicBoxTelemetryPanel runtime={device.runtimeInfo as MagicBoxRuntimeInfo | undefined} lastSeen={device.lastSeen} />

        <UptimeChart heartbeats={heartbeats} title="Uptime (24h)" height={180} />

        <div className="flex gap-4 min-h-0">
          {/* Left: Cameras */}
          <Card className="flex-1 min-w-0 flex flex-col bg-zinc-900/30 border border-white/5">
            <div className="p-4 border-b border-white/10 shrink-0">
              <h3 className="text-lg font-bold text-zinc-100">Cameras ({cameras.length})</h3>
            </div>
              ...
              {cameras.length === 0 ? (
                ...
                <div className="space-y-2">
                  {cameras.map((cam) => (
                    <Card
                      key={cam.id}
                      className="p-3 cursor-pointer transition-all bg-zinc-900/50 border border-white/5"
                      onClick={() => onSelectCamera(cam)}
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="text-zinc-100 truncate">{cam.name || cam.id}</div>
          ...
          <Card className="w-full sm:w-72 shrink-0 flex flex-col bg-zinc-900/50 border border-white/5">
            <div className="p-4 border-b border-white/10 shrink-0">
              <h3 className="text-lg font-bold text-zinc-100">Last 10 heartbeats</h3>
            </div>
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
              {(() => {
                const last10 = heartbeats.slice(-10).reverse();
                if (last10.length === 0) return <div className="text-zinc-500 text-sm">No heartbeats</div>;
                return (
                  <div className="space-y-1.5">
                    {last10.map((h, i) => (
                      <div key={`${h.timestamp}-${i}`} className="flex items-center justify-between gap-2 py-1.5 px-2 rounded bg-zinc-900/30 border border-white/5">
                        <span className="text-zinc-300 font-mono text-sm truncate">{formatTimeAgo(h.timestamp)}</span>
              ...
              <div className="pt-2 border-t border-white/10">
                <div className="text-xs text-zinc-500 mb-1.5">Info</div>
                {(() => {
                  const latest = heartbeats.length > 0 ? heartbeats[heartbeats.length - 1] : null;
                  if (!latest) return <div className="text-zinc-500 text-sm">—</div>;
