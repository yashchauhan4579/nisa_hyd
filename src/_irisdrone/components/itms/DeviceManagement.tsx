...
import type { WorkerWithCounts } from '@irisdrone/lib/worker-types';
    ...
    <ITMSLayout>
      <div className="h-full w-full flex flex-col lg:flex-row gap-4 p-4">
        ...
        <div className="w-full lg:w-96 flex flex-col gap-4 relative z-10">
          ...
          <h2 className="text-xl font-bold text-zinc-100 mb-4">Devices</h2>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm font-bold text-zinc-100">
                    {cleanDeviceName(device.name) || device.id}
                  </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            <Card className="bg-zinc-900/50 p-4">
              <h3 className="text-lg font-semibold text-zinc-100 mb-4">Device Information</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                  <div className="text-xs text-zinc-500 mb-1">Device ID</div>
                  <div className="text-zinc-300 font-mono">{device.id}</div>
                </div>
                <div>
                ...
                {assignedWorker && (
                  <div>
                    <div className="text-xs text-zinc-500 mb-1">Assigned Worker</div>
                    <div className="text-zinc-300 font-mono">{assignedWorker.name || assignedWorker.id}</div>
                  </div>
                )}
                <div>
                  <div className="text-xs text-zinc-500 mb-1">Created</div>
                  <div className="text-zinc-300 font-mono">{formatDateTime(device.createdAt)}</div>
                </div>
                <div>
                  <div className="text-xs text-zinc-500 mb-1">Updated</div>
                  <div className="text-zinc-300 font-mono">{formatDateTime(device.updatedAt)}</div>
                </div>
              </div>
            </Card>

            <Card className="bg-zinc-900/50 p-4">
              <h3 className="text-lg font-semibold text-zinc-100 mb-4">Health Metrics</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="text-center">
                  <div className="text-xs text-zinc-500 mb-1">Data Rate</div>
          ...
          <TabsContent value="config" className="space-y-4">
            <Card className="bg-zinc-900/50 p-4">
              ...
              <textarea
                ...
                rows={14}
                className="w-full bg-black/20 border border-white/10 text-zinc-300 font-mono text-sm rounded-lg p-3 resize-y focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500"
                spellCheck={false}
              />
            </Card>
            ...
            {device.metadata && Object.keys(device.metadata).length > 0 && (
              <Card className="bg-zinc-900/50 p-4">
                <h3 className="text-lg font-semibold text-zinc-100 mb-4">Metadata</h3>
                <pre className="w-full bg-black/20 border border-white/10 text-zinc-300 font-mono text-sm rounded-lg p-3 overflow-auto max-h-60">
            ...
            {device.rtspUrl && (
              <Card className="bg-zinc-900/50 p-4">
                <h3 className="text-lg font-semibold text-zinc-100 mb-4">RTSP Stream</h3>
                <div className="flex items-center gap-2">
                  <div className="text-xs text-zinc-500">URL</div>
                  <div className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-zinc-300 font-mono text-sm truncate">
                    {device.rtspUrl}
                  </div>
                </div>
              </Card>
            ...
            <Card className="bg-zinc-900/50 p-4">
              ...
              {device.latestEvent ? (
                <div className="space-y-2">
                  <div className="p-3 bg-zinc-900/30 rounded border border-white/10">
