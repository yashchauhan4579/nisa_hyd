...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@irisdrone/components/ui/dialog';
...
import { VCCReportPDF } from '@irisdrone/components/vcc/VCCReportPDF';
import { pdf } from '@react-pdf/renderer';
import { apiClient, type VCCStats, type VCCDeviceStats } from '@irisdrone/lib/api';
import { format } from 'date-fns';
import { DateTimeRangeContent } from '@irisdrone/components/vcc/DateTimeRangePicker';
import * as XLSX from 'xlsx';
import { LocationSelector } from '@irisdrone/components/vcc/LocationSelector';
...
export function VCCReportModal({ open, onOpenChange, cameras, initialDateRange, selectedCameraIds: propSelectedCameraIds = [] }: VCCReportModalProps) {
        ...
        try {
            ...
            if (selectedCameraIds.length === 1) {
                ...
                const params: any = {
                    ...
                    endTime: finalEnd.toISOString(),
                    groupBy: 'hour',
                    excludeDevicePrefix: 'NORMAL_VCC',
                };
    ...
    const handleGenerateExcel = async () => {
        try {
            ...
            if (selectedCameraIds.length > 0) {
                ...
                delete baseParams.excludeDevicePrefix;
    ...
    const handleDownloadExcel = () => {
        try {
            ...
            let fileName = `iris_tvcc_events_${format(new Date(), 'yyyy-MM-dd_HH-mm')}.xlsx`;
            ...
            if (selectedCameraIds.length === 1) {
                ...
                fileName = `iris_tvcc_events_${safeName}_${format(new Date(), 'yyyy-MM-dd_HH-mm')}.xlsx`;
            } else if (selectedCameraIds.length > 1) {
                fileName = `iris_tvcc_events_${selectedCameraIds.length}_cameras_${format(new Date(), 'yyyy-MM-dd_HH-mm')}.xlsx`;
            } else if (selectedLocations.length === 1) {
...
function PDFDownloadButton({ stats, dateRange, cameras, selectedCameraIds, selectedLocation }: {
    ...
    // Build the element fresh at click time. Wrapping it in useMemo + handing
    // it to pdf() sometimes hands react-pdf a stale fiber that can't be
    // re-rendered. Inlining at click time guarantees a brand-new element
    // each download.
    const buildDoc = () => {
    ...
    // Direct download — build the JSX inline at click time and pdf().toBlob()
    // it. Avoids PDFDownloadLink (caches blank blob from first render) and
    // useMemo'd elements (sometimes hand react-pdf a stale fiber).
    const [downloading, setDownloading] = useState(false);
    const handleDownload = async () => {
        console.log('[VCC PDF] click handler entered');
        try {
            setDownloading(true);
            console.log('[VCC PDF] building document');
            const docEl = buildDoc();
            console.log('[VCC PDF] document built, calling pdf().toBlob() …');
            const blob = await pdf(docEl).toBlob();
            console.log('[VCC PDF] blob ready, size=', blob.size);
            ...
            console.log('[VCC PDF] download triggered');
        } catch (err) {
            console.error('[VCC PDF] render failed:', err);
    ...
    return (
        <Button onClick={handleDownload} disabled={downloading} className="flex-1 w-full bg-amber-600 hover:bg-amber-700 text-white">
            {downloading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <FileDown className="w-4 h-4 mr-2" />}
