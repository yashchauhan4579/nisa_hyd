...
import { apiClient, type CameraHealth as APICameraHealth } from '@/lib/api';
import {
    Chart as ChartJS,
    CategoryScale,
    LinearScale,
...
export function CameraHealthPage() {
    ...
    const fetchHealth = async () => {
        try {
            const data = await apiClient.getCameraHealth();
            setCameras(data || []);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const handleManualSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await apiClient.addCameraHealthTarget(manualForm);
    ...
    const handleCsvSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            for (let i = startIndex; i < lines.length; i++) {
                ...
                if (ip && name) {
                    await apiClient.addCameraHealthTarget({ ip, name, location: location || '' });
                }
            }
            setShowAddModal(false);
            setCsvText('');
    ...
    const handleDelete = async (ip: string, e: React.MouseEvent) => {
        ...
        try {
            await apiClient.deleteCameraHealthTarget(ip);
            fetchHealth();
    ...
    return (
        <div className="p-6 h-full overflow-y-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className={cn("rounded-xl p-4 backdrop-blur-sm border", theme === 'light' ? 'bg-white border-gray-300' : 'bg-white/5 border-white/10')}>
            ...
            <div className={cn("rounded-xl p-6 backdrop-blur-sm border", theme === 'light' ? 'bg-white border-gray-300' : 'bg-white/5 border-white/10')}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredCameras.map((cam) => (
                        <div
                            ...
                            className={cn(
                                "p-4 rounded-xl border flex flex-col gap-3 backdrop-blur-sm hover:border-amber-500/50 transition-colors cursor-pointer group",
                                theme === 'light'
                        ...
                        >
                            ...
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <div className={cn("text-2xl font-mono font-bold", cam.latencyMs > 100 ? 'text-yellow-500' : theme === 'light' ? 'text-gray-900' : 'text-white')}>
                    ...
                    {cameras.length === 0 && (
                        <div className="col-span-full py-12 text-center text-gray-500 bg-white/5 rounded-xl border', theme === 'light' ? 'border-gray-300' : 'border-white/10', ' border-dashed">
            ...
            {
                selectedCamera && (
                    <div className={cn("fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm p-4", theme === 'light' ? 'bg-black/50' : 'bg-black/80')} onClick={() => setSelectedCamera(null)}>
                        <div className={cn("w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden border", theme === 'light' ? 'bg-white border-gray-300' : 'bg-gray-900 border-white/10')} onClick={e => e.stopPropagation()}>
                            <div className={cn("flex items-center justify-between p-6 border-b", theme === 'light' ? 'border-gray-300' : 'border-white/10')}>
                                <div>
                                    <h3 className={cn("text-xl font-bold flex items-center gap-3", theme === 'light' ? 'text-gray-900' : 'text-white')}>
                                ...
                                <button onClick={() => setSelectedCamera(null)} className={cn("p-2 rounded-lg transition-colors", theme === 'light' ? 'hover:bg-gray-100' : 'hover:bg-white/10')}>
                                    <X className="w-6 h-6 text-gray-400" />
                                </button>
                            </div>
                            <div className="p-6">
                                <div className={cn("h-80 w-full mb-6 rounded-xl p-4 border", theme === 'light' ? 'bg-gray-50 border-gray-300' : 'bg-black/20 border-white/5')}>
                                    <DetailedChart
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div className={cn("p-4 rounded-xl border", theme === 'light' ? 'bg-gray-50 border-gray-300' : 'bg-white/5 border-white/10')}>
                                        <div className="text-gray-400 text-xs uppercase mb-1">Current Latency</div>
                                        <div className={cn("text-2xl font-mono font-bold", theme === 'light' ? 'text-gray-900' : 'text-white')}>{selectedCamera.latencyMs} ms</div>
                                    </div>
                                    <div className={cn("p-4 rounded-xl border", theme === 'light' ? 'bg-gray-50 border-gray-300' : 'bg-white/5 border-white/10')}>
                                        <div className="text-gray-400 text-xs uppercase mb-1">Last seen</div>
                                        <div className={cn("text-lg font-medium", theme === 'light' ? 'text-gray-900' : 'text-white')}>{new Date(selectedCamera.lastPing).toLocaleString()}</div>
                                    </div>
                                    <div className={cn("p-4 rounded-xl border", theme === 'light' ? 'bg-gray-50 border-gray-300' : 'bg-white/5 border-white/10')}>
                                        <div className="text-gray-400 text-xs uppercase mb-1">Uptime</div>
                                        <div className={cn("text-lg font-medium", theme === 'light' ? 'text-gray-900' : 'text-white')}>
                                            {(() => {
            ...
            {/* Add Modal (Reused) */}
            {
                showAddModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className="w-full max-w-md bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border', theme === 'light' ? 'border-gray-300' : 'border-white/10', '">
                            {/* Header */}
                                ...
                                {activeTab === 'manual' ? (
                                    <form onSubmit={handleManualSubmit} className="space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <input type="text" required value={manualForm.ip} onChange={e => setManualForm({ ...manualForm, ip: e.target.value })} placeholder="192.168.1.100" className="w-full px-4 py-2 rounded-xl border', theme === 'light' ? 'border-gray-300' : 'border-white/10', ' bg-white/5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 text-white outline-none transition-all" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Approach Name</label>
                                            <input type="text" required value={manualForm.name} onChange={e => setManualForm({ ...manualForm, name: e.target.value })} placeholder="Main Entrance" className="w-full px-4 py-2 rounded-xl border', theme === 'light' ? 'border-gray-300' : 'border-white/10', ' bg-white/5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 text-white outline-none transition-all" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Location</label>
                                            <input type="text" value={manualForm.location} onChange={e => setManualForm({ ...manualForm, location: e.target.value })} placeholder="Entrance" className="w-full px-4 py-2 rounded-xl border', theme === 'light' ? 'border-gray-300' : 'border-white/10', ' bg-white/5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 text-white outline-none transition-all" />
                                        </div>
...
10.0.0.5, Lobby Camera, Int`}
                                                className="w-full px-4 py-3 rounded-xl border', theme === 'light' ? 'border-gray-300' : 'border-white/10', ' bg-white/5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 text-white outline-none transition-all font-mono text-sm"
                                            />
