...
import { apiClient, type Device, type CameraHealth } from '@/lib/api';
import { Camera, ChevronDown, ChevronRight, LayoutGrid, Maximize2, Minimize2, Shield } from 'lucide-react';
import { useCameraGrid } from '@/contexts/CameraGridContext';
import { cn } from '@/lib/utils';
import { WhepPlayer } from '@/components/video/WhepPlayer';
import { PingTerminal } from '@/components/cameras/PingTerminal';
...
export function CameraView() {
  ...
  useEffect(() => {
    const fetchCameras = async () => {
      try {
        setLoading(true);
        const healthData = await apiClient.getCameraHealth();
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 flex flex-col overflow-hidden" ref={(node) => {
        ...
        {/* Grid Container */}
        <div className={cn(
          "flex-1 p-0.5 overflow-auto",
          theme === 'light' ? 'bg-white' : 'bg-black/20'
        )}>
          <div
            className="grid gap-[3px] h-full transition-all duration-300 ease-in-out"
            style={{
              ...
              >
                {slot.device ? (
                  <>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/90 to-transparent p-2 transition-opacity duration-200 flex items-end justify-between">
                      <div
                        className="text-white/90 text-xs font-medium px-1 truncate cursor-pointer hover:underline hover:text-amber-300 transition-colors"
                        onClick={(e) => { e.stopPropagation(); if (slot.device) setPingIp(slot.device.id); }}
                        title="Click to ping this IP"
                      >
                        {slot.device.id}
                      </div>
                  ...
                  <div className="w-full h-full flex items-center justify-center opacity-30 hover:opacity-50 transition-opacity group-hover:border-amber-500/30">
      ...
      {/* Ping Terminal Modal */}
      {pingIp && (
        <PingTerminal ip={pingIp} onClose={() => setPingIp(null)} />
      )}
    </div>
  );
}
