...
import { toIST, utcHourToIST } from '@/lib/dateUtils';
import {
    Chart as ChartJS,
    Tooltip,
    Legend,
...
interface VCCHeatmapProps {
    stats: VCCStats | null;
    loading: boolean;
    // 'date' (DD.MM.YYYY, default) or 'weekday' (Mon/Tue/…) for the Y-axis day labels.
    dayFormat?: 'date' | 'weekday';
    // Render without the glass card chrome so it blends into the page.
    seamless?: boolean;
}

export function VCCHeatmap({ stats, loading, dayFormat = 'date', seamless = false }: VCCHeatmapProps) {
    ...
    const data = useMemo(() => {
        ...
        stats.byTime.forEach((item: any) => {
            ...
            const displayLabel = dayFormat === 'weekday'
                ? new Intl.DateTimeFormat('en-US', { weekday: 'short', timeZone: 'UTC' }).format(istDate) // Mon, Tue…
    ...
    const Wrapper: any = seamless ? 'div' : Card;
    return (
        <Wrapper className={seamless ? "relative" : "glass p-4 relative overflow-hidden group/chart hover:border-white/20 transition-all duration-300"}>
            {!seamless && <div className="absolute -bottom-20 left-1/2 -translate-x-1/2 w-64 h-64 bg-green-500 blur-[100px] opacity-[0.05] pointer-events-none" />}
