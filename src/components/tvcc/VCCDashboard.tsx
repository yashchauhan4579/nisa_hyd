...
import { MultiCameraSelector, type CameraOption } from '@/components/tvcc/MultiCameraSelector';
import { VCCInsights } from '@/components/tvcc/VCCInsights';
import { VCCHeatmap } from '@/components/tvcc/VCCHeatmap';
import { VCCDevicesView } from '@/components/tvcc/VCCDevicesView';
...
import { LocationSelector } from '@/components/tvcc/LocationSelector';
...
export function VCCDashboard() {
  ...
  useEffect(() => {
    const fetchCameras = async () => {
      try {
        ...
        // Exclude Normal VCC cameras — those belong to the NVCC dashboard
        const thermalCameras = response.cameras.filter(c =>
          !c.id.toUpperCase().startsWith('NORMAL_VCC') &&
          !c.workerId?.toUpperCase().startsWith('NORMAL_VCC')
        );
        setCameras(thermalCameras.map(c => ({
          id: c.id,
          name: c.name,
  ...
  const fetchStats = async (silent = false) => {
    try {
      ...
      if (selectedCameraIds.length > 0) {
        ...
        // Fetch all cameras stats (Global) — exclude Normal VCC devices
  ...
  const fetchRealtime = async () => {
    try {
      ...
      const data = await apiClient.getVCCRealtime({ excludeDevicePrefix: 'NORMAL_VCC' });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchRealtime();
      // Refresh today's stats every 5 seconds for live updates in vehicle type distribution
      fetchTodayStats(true);
      // Don't refresh historical stats aggressively to prevent flicker
    }, 5000);
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-3 bg-background/50">
      {/* Header Container */}
      <div className={cn(
        "px-4 pt-3 pb-3 rounded-xl mb-4 relative overflow-hidden border shadow-lg space-y-2",
        isLight ? "bg-white border-gray-200" : "glass border-black/5 dark:border-white/5"
      )}>
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {selectedCameraIds.length === 1 && (() => {
                ...
                return cam ? (
                  <p className={cn("text-xs mt-0.5 truncate max-w-[300px]", isLight ? "text-gray-500" : "text-amber-200/60")}>
                    {cam.name.replace(/^Camera\s+/i, "")}
                  </p>
                ) : null;
              })()}
              {selectedCameraIds.length > 1 && (
                <p className={cn("text-xs mt-0.5", isLight ? "text-gray-500" : "text-amber-200/60")}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className={cn("text-xs font-medium", isLight ? "text-gray-700" : "text-gray-200")}>Health Report</span>
            </Button>
            {stats && (
              <Button
                variant="outline"
        ...
        <div className="flex items-center gap-2 relative z-10 flex-wrap">
          ...
          <div className={cn("w-px h-5", isLight ? "bg-gray-200" : "bg-black/10 dark:bg-white/10")} />
          <DateTimeRangePicker
            value={dateRange}
      ...
      {/* New KPI Stats Row */}
      <div className="mb-4">
        <KPIStats stats={stats} realtime={realtime} loading={statsLoading} totalCameras={94} isSingleCamera={selectedCameraIds.length === 1} />
      </div>
