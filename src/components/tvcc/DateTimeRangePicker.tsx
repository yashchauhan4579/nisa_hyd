        ...
        <Popover open={open} onOpenChange={(o) => {
            ...
            <PopoverTrigger asChild>
                <Button
                    variant="outline"
                    className={cn(
                        "justify-start text-left font-normal h-8 px-3 !bg-white !border-slate-200 !text-slate-700 hover:!bg-slate-50 hover:!text-slate-900 dark:!bg-white/5 dark:!border-white/10 dark:!text-white dark:hover:!bg-white/10 dark:hover:!text-white",
                        !value && "text-muted-foreground",
                        className
                    )}
                >
                    <CalendarIcon className="mr-2 h-4 w-4 !text-slate-500 dark:!text-zinc-400" />
                    <span className="text-xs truncate max-w-[280px] !text-slate-700 dark:!text-white">
                        {format(value.startDate, "MMM d, yyyy HH:mm")} - {format(value.endDate, "MMM d, yyyy HH:mm")}
                    </span>
                    <ChevronDown className="ml-2 h-3 w-3 !text-slate-400 dark:!text-zinc-500" />
                </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0 min-w-[520px] bg-white/95 text-slate-900 border-slate-200 dark:bg-zinc-950/95 dark:text-white dark:border-white/10" align="end">
