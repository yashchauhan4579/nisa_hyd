...
export function VCCInsights({ stats, overallStats, loading, cameras, isSingleCamera, hourlyStats, healthStats, variant = 'itms' }: VCCInsightsProps) {
    if (loading || !stats) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                {[0, 1, 2, 3].map((i) => (
                    <Card key={i} className="glass p-4 relative overflow-hidden">
                        <div className="flex flex-row items-center justify-between">
                            ...
                            <div className="w-14 h-14 rounded-2xl bg-white/10 animate-pulse relative z-10 shrink-0" />
                        </div>
                    </Card>
                ))}
            </div>
    ...
    const getValueClassName = (insight: { title: string; value: string | number }) => {
        const baseClass = "font-bold tabular-nums tracking-tight mt-0.5 leading-tight";
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            {insights.map((insight, idx) => (
                <Card key={idx} className="glass p-4 flex flex-row items-center justify-between relative overflow-hidden group hover:scale-[1.02] transition-transform duration-200">
                    {/* Subtle glow */}
                    <div className={cn("absolute -top-6 right-0 w-24 h-24 rounded-full blur-2xl opacity-20 group-hover:opacity-30 transition-opacity", insight.color.replace('text-', 'bg-'))} />
                    ...
                    {/* Right: large icon */}
                    <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center relative z-10 shrink-0", insight.bgColor, insight.color)}>
