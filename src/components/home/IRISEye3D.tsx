...
interface EyeProps {
  color: string;
  isActive: boolean;
  // Where the iris should look. Driven by the cursor while it moves, and by
  // involuntary random saccades (managed in IRISEye3D) when the cursor is idle.
  lookRef: MutableRefObject<{ x: number; y: number }>;
}

function RealisticEye({ color, isActive, lookRef }: EyeProps) {
  ...
  useFrame((state) => {
    ...
    // Iris chases the current look target (cursor while moving, random saccades when idle)
    if (irisRef.current && pupilRef.current) {
      irisRef.current.position.x += (lookRef.current.x - irisRef.current.position.x) * 0.08;
      irisRef.current.position.y += (lookRef.current.y - irisRef.current.position.y) * 0.08;
...
export function IRISEye3D({ color = '#f97316', isActive = false, size = 160 }: IRISEye3DProps) {
  ...
  // The look target the iris chases, updated via a ref (no re-render per move).
  const lookRef = useRef({ x: 0, y: 0 });
  // Timestamp of the last cursor movement; drives "following" vs "idle" mode.
  const lastMoveRef = useRef(0);

  // Follow the cursor while it is moving.
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      // Direction from the eye centre to the cursor, normalised by half-viewport
      // then scaled to how far the iris may travel inside the sclera.
      const nx = clamp((e.clientX - cx) / (window.innerWidth / 2));
      const ny = clamp((e.clientY - cy) / (window.innerHeight / 2));
      lookRef.current = { x: nx * 0.42, y: -ny * 0.32 };
  ...
  // When the cursor has been still for a beat, resume involuntary eye saccades.
  useEffect(() => {
    const IDLE_MS = 1600;
    const id = setInterval(() => {
      if (performance.now() - lastMoveRef.current > IDLE_MS) {
        lookRef.current = {
  ...
  return (
    <div ref={wrapRef} style={{ width: size, height: size, borderRadius: '50%', overflow: 'hidden' }}>
      <Canvas
      ...
      >
        ...
        <RealisticEye color={color} isActive={isActive} lookRef={lookRef} />
