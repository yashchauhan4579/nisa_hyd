...
// From 21st.dev (easemize/multi-type-ripple-buttons). Click ripple on every
// variant, plus 'hover', 'ghost', and 'hoverborder' grid-hover effects.
      ...
      {jsRipples.map(ripple => (
        <span
          ...
          style={{
            ...
            ['--ripple-duration' as never]: `${rippleDuration}ms`,
          } as CSSProperties}
        />
      ))}
    </div>
  ...
  if (variant === 'hover') {
    ...
    return (
      <>
        ...
        <button type={type} className={cls} onClick={handleButtonClick} disabled={disabled}>
    ...
    return (
      <>
        ...
        <button type={type} className={cls} onClick={handleButtonClick} disabled={disabled}>
          ...
          >
            {Array.from({ length: GRID_HOVER_TOTAL_CELLS }, (_, index) => (
              <span key={index} className="hoverborder-variant-grid-cell relative flex justify-center items-center pointer-events-auto" />
            ))}
