  ...
  useEffect(() => {
    const fetchDevices = async () => {
      try {
        ...
        const cachedDevices = await getDevices();
        // Convert to DeviceMapMarker format (minimal fields)
        const mapDevices: DeviceMapMarker[] = cachedDevices.map(d => ({
          id: d.id,
          name: d.name,
          type: d.type,
          lat: d.lat,
    ...
    fetchDevices();
    // Refresh devices every 30 seconds (will use cache if not expired)
    const interval = setInterval(() => getDevices(true).then(cachedDevices => {
      const mapDevices: DeviceMapMarker[] = cachedDevices.map(d => ({
        id: d.id,
        name: d.name,
        type: d.type,
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 glass rounded-2xl p-5 shadow-2xl min-w-[350px] max-w-[400px] border border-white/20 dark:border-white/10">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg ${selectedHotspot.type === 'CAMERA' ? 'bg-amber-500/10' :
                  ...
                  {(() => {
                    ...
                    return (
                      <IconComponent className={`w-5 h-5 ${selectedHotspot.type === 'CAMERA' ? 'text-amber-500' :
