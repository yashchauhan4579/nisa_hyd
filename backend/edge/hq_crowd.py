...
heads=[xy[j].tolist() for j in range(len(cls)) if cls[j]==0]
nw=1280; nh=int(frame.shape[0]*nw/frame.shape[1]); sx=nw/frame.shape[1]; sy=nh/frame.shape[0]
hq=cv2.resize(frame,(nw,nh)); cv2.imwrite("/tmp/hqc_frame.jpg",hq,[cv2.IMWRITE_JPEG_QUALITY,92])
hb=[[b[0]*sx,b[1]*sy,b[2]*sx,b[3]*sy] for b in heads]
json.dump({"heads":hb},open("/tmp/hqc_boxes.json","w"))
print("crowd hq %dx%d heads=%d"%(nw,nh,len(heads)))
